...
list contains `$extra_args`, the color argument is injected there; otherwise
it is added to the end of the command line.

```toml
[merge-tools.git]
diff-args = ["$extra_args", "$left", "$right"]
enable-color-args = ["--color=always"]
disable-color-args = ["--color=never"]
```

`jj`'s built-in diff tool has support for suppressing whitespace
changes. Config settings are available to pass these settings through to
external tools: the `ignore-all-space-args` config is used when
`--ignore-all-space` is set, and `ignore-space-change-args` is used for
`--ignore-space-change`.

```toml
[merge-tools.git]
ignore-all-space-args = ["--ignore-all-space"]
ignore-space-change-args = ["--ignore-space-change"]
```
